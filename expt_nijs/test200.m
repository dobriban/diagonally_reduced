        ...
        err_fr_means = zeros(ngams,ndeltas);
        err_fr_devs = zeros(ngams,ndeltas);
%
        errs_op_cube = zeros(ngams,ndeltas,nmonte);
        err_op_means = zeros(ngams,ndeltas);
        err_op_devs = zeros(ngams,ndeltas);

        deltas = linspace(.1,1,ndeltas)
        gams = linspace(.1,.9,ngams)
...
%
        ...
        errs_fr_cube(igam,idel,:) = errs_fr;

        err_fr_means(igam,idel) = mean(errs_fr);
        err_fr_devs(igam,idel) = sqrt(var(errs_fr));
%
%   operator norm
%
        errs_op = test_avail_monte(gam,n,k,delta,nmonte,'o');
        errs_op_cube(igam,idel,:) = errs_fr;

        err_op_means(igam,idel) = mean(errs_op);
        err_op_devs(igam,idel) = sqrt(var(errs_op));
