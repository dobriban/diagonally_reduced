...
%
        niter=20
        evals = zeros(ndels,nvals);
        dels = linspace(.1,.9,ndels)
        ev_plus = linspace(2,120,nvals)

        err_mean_shr = zeros(ndels,nvals);
        err_mean_opt = zeros(ndels,nvals);
%
        err_sdev_shr = zeros(ndels,nvals);
        err_sdev_opt = zeros(ndels,nvals);
...
%
        ...
        for ival=1:nvals
%
        evmin = 1 + (sqrt(gam) + 1)/delta + ev_plus(ival);
        evals(idel,ival) = evmin;

        [errs_shr,errs_opt] = test_compl_monte(delta,nmonte,evmin,gam,m,...
           n,k,niter);
%
        err_mean_shr(idel,ival) = mean(errs_shr);
        err_mean_opt(idel,ival) = mean(errs_opt);

        err_sdev_shr(idel,ival) = sqrt(var(errs_shr));
        err_sdev_opt(idel,ival) = sqrt(var(errs_opt));
...
%
%
        function [errs_shr,errs_opt] = test_compl_monte(delta,nmonte,...
           evmin,gam,m,n,k,niter)
%
%

        errs_shr = zeros(1,nmonte);
        errs_opt = zeros(1,nmonte);

        evs_noisy = evmin + [k-1:-1:0]
%
        for ijk=1:nmonte

        [xs_clean,u,s,v,rnoise,xs,ys,inds,rcov,rcov2,ells] = ...
           prep_gauss(m,n,k,delta,evs_noisy);

        ss = sum(ells);
%
        [x_shr,uy,vy,sy_sh,sy] = svd_shrink_mat(ys,m,n,delta,k,'f');
%
        tol=[];
        [x_opt,u_opt,s_opt,v_opt] = call_optspace(ys,inds,m,n,niter,tol,k);


        errs_shr(ijk) = norm(x_shr - xs_clean,'fro')^2 / n / ss;
        errs_opt(ijk) = norm(x_opt - xs_clean,'fro')^2 / n / ss;
